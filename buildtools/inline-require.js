 ...
 */
let inlineRequiredDependencyMap
    ...
    visitor: {
      ...
      CallExpression: function (path) {
        ...
        if (isTopLevelRequireAlias(path)) {
          ...
          const maybePaths = identifierToPathsMap[varName]
...
function isLodash(name) {
  ...
  if (!!inlineRequiredDependencyMap[name]) {
    const moduleName = inlineRequiredDependencyMap[name].toString().toLowerCase()
